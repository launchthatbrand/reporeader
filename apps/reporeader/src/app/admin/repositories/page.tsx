...
export default function AdminRepositoriesPage() {
  const repos = useQuery(api.repos.listReposForCurrentUser, {});
  ...
  const handleAddRepo = async () => {
    try {
      ...
      setStatusMessage("Repository saved to your workspace.");
    } catch (error) {
    ...
    <section className="flex w-full flex-1 flex-col gap-6">
      <header>
        ...
        <p className="mt-3 text-sm text-muted-foreground">
          Repositories owned by your user account for personal run orchestration.
        </p>
      </header>
        ...
        <div className="mt-3 overflow-x-auto">
          ...
          {sortedRepos.length === 0 ? (
            <p className="py-6 text-sm text-muted-foreground">
              No repositories assigned to your account yet.
            </p>
          ) : null}
        </div>
      </section>
